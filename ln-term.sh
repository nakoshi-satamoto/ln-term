...
./lightning-cli listpeers | jq '.[] | .[] | select(last(.channels[].state_changes[].new_state) != "ONCHAIN") | (.channels[].to_us_msat / 1000)' | jq -s 'add' | awk -F "." {'print $1'} | tr -d '\n' && echo -n " ≐" && echo " | If you see ⚠INACCURATE_STATE⚠ for any of the chans,"
echo -n "Total spendable (out) sats in open chans: "
./lightning-cli listpeers | jq '.[] | .[] | select(last(.channels[].state_changes[].new_state) != "ONCHAIN") | (.channels[].spendable_msat / 1000)' | jq -s 'add' | awk -F "." {'print $1'} | tr -d '\n' && echo -n " ≐" && echo " | these totals should be considered inaccurate also."
echo -n "Total receivable (in) sats in open chans: "
./lightning-cli listpeers | jq '.[] | .[] | select(last(.channels[].state_changes[].new_state) != "ONCHAIN") | (.channels[].receivable_msat / 1000)' | jq -s 'add' | awk -F "." {'print $1'} | tr -d '\n' && echo -n " ≐" && echo " | Read the documentation in this script for more info."
...
echo "                                                                      ₿≐≐≐≐≐≐≐≐mmm       ₿≐≐≐≐≐≐≐≐mmm  |"
./lightning-cli listpeers | jq '.[] | .[] | select(last(.channels[].state_changes[].new_state) != "ONCHAIN") | .id + " | " + "█" + (.channels[].spendable_msat | tostring | (length | if . >=11 then "" else "*" * (11 - .) end) as $padding | "\($padding)\(.)") + "█" + "<-⚡->" + "█" + (.channels[].receivable_msat | tostring | (length | if . >=11 then "" else "*" * (11 - .) end) as $padding | "\($padding)\(.)") + "█" + " | " + (.channels[].to_us_msat | tostring | (length | if . >=11 then "" else "*" * (11 - .) end) as $padding | "\($padding)\(.)") + " | " + .channels[].short_channel_id + " | " + .channels[].state + " " + (.connected | tostring)'  | awk -F " | " '!seen[$1]++' | sed -e 's/ONCHAIN/⚠INACCURATE_STATE⚠/g' | sort | sed -e 's/ true/ connected/g' | sed -e 's/ false/ ⚠DISCONNECTED⚠/g' \
| sed \
